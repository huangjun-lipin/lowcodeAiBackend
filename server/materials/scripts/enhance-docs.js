...
const components = [
  ...
  { name: 'container', docName: 'container' },
  { name: 'menu', docName: 'menu' },
  { name: 'page-header', docName: 'pageheader' }
];

async function enhanceComponents() {
  console.log('开始增强 fusion-ui 组件文档...');
  
  for (const comp of components) {
    const componentPath = `/Users/junhuang/fe/lowcode-materials/packages/fusion-ui/src/components/${comp.name}`;
    const docPath = `./fusion-ui/${comp.docName}.md`;
    ...
    try {
      ...
      if (enhancedContent.trim()) {
        ...
        // 检查是否已经包含源码分析内容
        if (content.includes('## 核心接口定义') || content.includes('## 属性配置')) {
  ...
  console.log('fusion-ui 组件文档增强完成！');
}

if (require.main === module) {
  enhanceComponents().catch(console.error);
}

module.exports = { enhanceComponents };